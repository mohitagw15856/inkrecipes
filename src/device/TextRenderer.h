...
// docs/INKKIT_GAPS.md), so InkRecipes brings its own compact 5x7 bitmap font and
// word wrapping. The wrapping itself lives in the host-tested core
// (inkrecipes::wrapText); this class only rasterises glyphs.
//
...
class TextRenderer {
 public:
  ...
  // Integer-scaled variants for the big cooking-mode type.
  int glyphScaled(int x, int y, char c, int scale);
  int textScaled(int x, int y, const std::string& s, int scale);
  int lineHeightScaled(int scale) const;
