...
int TextRenderer::glyphScaled(int x, int y, char c, int scale) {
  const auto code = static_cast<uint8_t>(c);
  const uint8_t* g;
  if (code >= kFontFirst && code <= kFontLast) {
    g = kFont5x7[code - kFontFirst];
  ...
  return (kFontWidth + kGlyphGap) * scale;
}

int TextRenderer::textScaled(int x, int y, const std::string& s, int scale) {
  int cx = x;
  for (char c : s) cx += glyphScaled(cx, y, c, scale);
  return cx - x;
}

int TextRenderer::lineHeightScaled(int scale) const {
  return (kFontHeight + kLineGap) * scale;
}
